proposals([]).
desiredProducts(["Banana","Apple","Guava"]).
!askForProposals.
...
@r1 +!askForProposals : true <- 
	?desiredProducts(LDP);
	for (.member(Item,LDP)) {
		if (.random(W) & W >= 0.5) {//It is just a random decision, what I want?
...
@p3 +noProposal(Product)[source(Supplier)] : true <-
	.print("A rejection was received from ",Supplier,": product ''",Product ,"''").
